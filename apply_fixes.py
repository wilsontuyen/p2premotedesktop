...
# 3. Fix async download globally (GUI mode)
old_async = """            import platform
...
new_async = """            if dest_dir and os.path.isdir(dest_dir):
                self.ignore_destroy_clipboard = True
                import threading
                threading.Thread(target=self._render_format_process, args=(dest_dir,), daemon=True).start()
                return
...
# 4. Fix batch_start double dialog (GUI mode)
...
# 5. Fix _agent_mouse_poll_loop (Headless mode)
old_agent_mouse = """            try:
...
# 6. Fix re_setup_agent (Headless mode)
old_agent_re_setup = """                dummy_h = create_hdrop_data(["C:\\RemoteDesktop_Paste_Trigger.tmp"])
                ...
                # Lập lịch setup lại delayed rendering sau 200ms
                def re_setup_agent():
                    time.sleep(0.2)
...
# 7. Fix Headless DownPipe START duplicate dialog
